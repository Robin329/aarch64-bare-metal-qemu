		    ...
		    int size, int precision, int type)
{
  ...
  */
		...
		do {
			tmp[i++] =
				(digits[((unsigned char)num) & mask] | locase);
			num >>= shift;
		} while (num);
	} else { /* base 10 */
...
static char *mac_address_string(char *buf, char *end, unsigned char *addr,
...
static char *ip6_addr_string(char *buf, char *end, unsigned char *addr,
			     int field_width, int precision, int flags)
{
...
static char *ip4_addr_string(char *buf, char *end, unsigned char *addr,
			     int field_width, int precision, int flags)
{
		     ...
		     int field_width, int precision, int flags)
{
   ...
   */
	...
	case 'm':
		...
		/* Fallthrough */
	case 'M':
		return mac_address_string(buf, end, ptr, field_width, precision,
					  flags);
	case 'i':
		flags |= SPECIAL;
		/* Fallthrough */
	...
	case 'U':
		return uuid_string(buf, end, ptr, field_width, precision, flags,
				   fmt);
#endif
	default:
		break;
