...
static void init_gicd(void)
{
	...
	uart_puthex(readl_relaxed(GIC_GICD_TYPER));
	uart_puts("GICD_IIDR:");
	uart_puthex(readl_relaxed(GIC_GICD_IIDR));
	uart_puts("irqs:");
	uart_putdec(GICD_TYPER_IRQS((readl_relaxed(GIC_GICD_TYPER))));
	/* Diable distributor */
	*REG_GIC_GICD_CTLR = GIC_GICD_CTLR_DISABLE;

	/* Disable all IRQs */
	regs_nr =
		(GIC_INT_MAX + GIC_GICD_INT_PER_REG - 1) / GIC_GICD_INT_PER_REG;
	uart_puts("regs_nr:");
	uart_putdec(regs_nr);
	for (i = 0; regs_nr > i; ++i)
	...
	for (i = GIC_INTNO_PPI0 / GIC_GICD_ICFGR_PER_REG;
	     (GIC_INT_MAX + (GIC_GICD_ICFGR_PER_REG - 1)) /
		     GIC_GICD_ICFGR_PER_REG >
	     i;
	     ++i)
...
static void gicd_set_target(irq_no irq, uint32_t p)
{
	...
	shift = (irq % GIC_GICD_ITARGETSR_PER_REG) *
		GIC_GICD_ITARGETSR_SIZE_PER_REG;
...
static void gicd_set_priority(irq_no irq, uint32_t prio)
{
	...
	shift = (irq % GIC_GICD_IPRIORITY_PER_REG) *
		GIC_GICD_IPRIORITY_SIZE_PER_REG;
 ...
 */
int gic_v3_find_pending_irq(struct _exception_frame *exc
